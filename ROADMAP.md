...
- [ ] 保留原始 Git 历史并创建升级分支；
- [ ] 记录原项目文件、权重、数据和自定义源码的来源；
- [ ] 将论文和历史图表迁移到 `docs/archive/`；
- [ ] 识别并删除缓存、重复依赖和明确无用文件；
- [ ] 冻结 Ultralytics 版本和第三方许可证清单；
- [ ] 修复训练脚本本地包加载不一致；
- [ ] 将 NWD 真正接入 bbox loss，或从模型声明中移除；
- [ ] 将单个伪“消融脚本”改成配置驱动实验入口；
- [ ] 建立 `pyproject.toml`、锁定依赖、代码格式和测试配置；
- [ ] 建立 GitHub Actions 或等价 CI；
- [ ] 添加自定义模块形状、梯度、奇数尺寸和导出测试；
- [ ] 建立原始权重离线推理烟雾测试；
- [ ] 建立风险、决策和变更日志。
...
BoT-SORT 运动分支与视觉 GMC 已进入云端验收：实现前景框屏蔽的稀疏光流、RANSAC 仿射估计、质量与
运动合理性回退门控、逐帧审计字段，以及保持其余参数一致的无 GMC/有 GMC 合成平移消融。真实海域
收益验证通过前，本任务保持未勾选；ReID 外观分支按后续质量门控任务单独实现。
...
- [ ] 建立 ReID 质量门控；
- [ ] 聚合 Tracklet 中清晰帧形成外观模板；
- [ ] 对极小目标优先使用运动和世界坐标，而非强行 ReID；
- [ ] 建立短时遮挡轨迹插值和有限外推；
...
- [ ] 定义相机内参、畸变和外参标定流程；
- [ ] 接入 MAVLink 或实际无人机遥测格式；
- [ ] 完成视频与遥测时钟同步和延迟标定；
- [ ] 建立 ENU/NED、相机、云台和图像坐标转换；
- [ ] 使用视线与海平面相交估算地理位置；
- [ ] 处理海拔、潮位、浪高和姿态误差；
- [ ] 使用 EKF/UKF 平滑位置、速度与协方差；
- [ ] 输出接近、远离、横向和静止趋势；
- [ ] 输出 TTC，并在低质量时降级为相对趋势；
