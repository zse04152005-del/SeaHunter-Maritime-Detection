...
- [x] 使用经纬度或局部海平面多边形定义区域；
- [x] 支持进入、离开、逗留、接近、逆向和碰撞趋势；
- [x] 引入进入/退出滞回、最短停留和冷却期；
- [x] 建立轨迹可靠度、TTC、目标类型和区域等级风险评分；
- [x] 建立 `open/update/acknowledged/closed` 生命周期；
- [x] 保存告警前后环形视频、轨迹、遥测和模型版本；
- [x] 提供 REST/WebSocket/MQTT 输出；
- [x] 提供操作员确认、误报标记和反馈回流。
...
- [x] 制定检测、Track ID、遮挡、天气、海况和事件标注规范；
- [x] 按航次、日期、地点划分训练/验证/测试；
- [x] 建立像素尺寸分桶和长尾类别报告；
- [x] 建立海浪、尾流、鸟、泡沫、反射等硬负样本集；
- [x] 构造真实与合成退化强度分级；
- [x] 建立主动学习：低置信、分歧、断轨和误告警采样；
- [x] 建立教师-学生半监督基线；
- [x] 使用时间一致性和 Tracklet 一致性过滤伪标签；
- [x] 进行置信度校准和天气分层阈值评估；
- [x] 接入 DVC/lakeFS 数据版本与 MLflow 实验追溯接口；生产仓库、凭据和真实运行记录为部署门禁。
...
- [x] 低分辨率全局扫描；
- [x] 基于 objectness、运动残差、历史轨迹和丢失轨迹生成 ROI；
- [x] Top-K 高分辨率细查；
- [x] 周期性全局刷新避免盲区；
- [x] 跨尺度融合和边界目标去重；
- [x] 根据延迟、温度和队列动态调节 K 与分辨率；
- [ ] 对比全帧 1024、固定切片和选择性 ROI 的精度/延迟；BLOCKED：需要冻结真实数据和目标硬件。

#### M6.2 时序小目标动态监测头（P0）

- [x] 在稳像特征上计算多帧变化；
- [x] 融合当前空间特征、运动残差和历史特征；
- [x] 对相机运动、海浪周期运动和真实目标运动进行区分；
- [x] 保持有限状态，支持断流和场景切换时清空；
- [ ] 对比单帧头、帧差分和时序头；BLOCKED：需要冻结真实视频测试集和训练结果。

#### M6.3 检测与跟踪联合置信度（P0）

- [x] 融合检测概率、轨迹先验、运动一致性、ReID 和状态不确定度；
- [x] 使用温度缩放或等价方法校准检测概率；
- [x] 对周期性海浪假目标加入背景/运动否决；
- [ ] 评估低置信目标召回、误报持续时间和断轨率；BLOCKED：需要冻结真实检测/跟踪测试集。
- [x] 禁止仅凭重复出现无限抬升置信度。

#### M6.4 频域与空间域联合降采样（P1）

- [x] 实现可导出的固定高低通与抗混叠联合降采样参考；
- [x] 避免依赖复杂 FFT/复数算子进入首版边缘引擎；
- [ ] 对比 SPDConv、标准步长卷积和联合降采样；BLOCKED：需要完成真实训练消融。
- [ ] 单独统计超小目标、海浪硬负样本和 TensorRT 延迟；BLOCKED：需要真实数据和 NVIDIA 目标硬件。

#### M6.5 退化天气路由与 Sim2Real（P2）

- [ ] 先训练单一鲁棒模型；BLOCKED：需要生产训练数据与 GPU 训练。
- [ ] 训练轻量退化类型/强度分类器；BLOCKED：需要带退化标签的真实数据。
- [x] 先路由预处理与阈值配置，再考虑路由完整专家模型；
- [x] 路由按视频片段而非逐帧抖动切换；
- [x] 只有真实数据覆盖不足时才引入 Sim2Real；
- [ ] 专家收益必须覆盖显存、切换和维护成本；BLOCKED：需要目标硬件成本/收益实测。
...
- [x] 导出动态/静态 shape ONNX 并进行数值一致性测试；
- [ ] 构建 TensorRT FP16 引擎；BLOCKED：需要 NVIDIA/TensorRT 目标构建机。
- [ ] 使用真实海况校准集评估 INT8；BLOCKED：需要冻结真实校准集与目标硬件。
- [ ] 将视频链路迁移到 DeepStream/C++ 或等价高性能运行时；BLOCKED：配置模板和能力门禁已完成，仍需目标镜像集成。
- [ ] 使用 NVDEC、CUDA stream 和预分配内存；BLOCKED：需要在 NVIDIA 目标设备证明实际生效。
- [x] 建立热降频、显存不足和推理超时降级策略；
- [x] 模型包签名并包含完整 `ModelManifest`；
- [x] 支持灰度部署、健康检查、失败回滚；
- [ ] 完成 8 小时性能测试和 72 小时稳定性测试；BLOCKED：需要目标设备连续运行 8/72 小时。
- [ ] 测试断网、断流、坏帧、时钟漂移和进程重启；BLOCKED：云端已验证故障矩阵合同，仍需目标链路注入实测。
...
- [x] 建立海试覆盖、盲测、指标、证据、四域审查和 V1 冻结的机器门禁；模板不会被当作验收证据；
- [ ] 至少覆盖多个地点、时间段、高度和海况；BLOCKED：需要真实海试计划、飞行许可、人员与场地。
- [ ] 执行正常、低照、雾、眩光和高浪场景矩阵；BLOCKED：需要真实天气/海况航次。
- [ ] 组织操作员盲测和告警确认；BLOCKED：需要独立操作员和冻结注入/真值方案。
- [ ] 验证摄像机标定、遥测同步和区域配置流程；BLOCKED：云端已验证工具合同，仍需逐设备现场流程。
- [ ] 统计误报/小时、漏警、重捕获和距离误差；BLOCKED：需要冻结真实事件与 M3 真值集。
- [ ] 验证日志、证据包、模型版本和事件审计；BLOCKED：云端工程链路已通过，仍需海试证据逐场审计。
- [ ] 完成安全、隐私、许可证和运维评审；BLOCKED：工程清单已完成，仍需独立具名审批和法律意见。
- [ ] 冻结 V1.0 模型、引擎、镜像和文档；BLOCKED：发布门禁会拒绝冻结，直至全部数据/硬件/海试/审查证据通过。
